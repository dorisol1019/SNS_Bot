...
namespace Mastonet
{
    public partial class MastodonClient
    {
        ...
        public async Task<Auth> Connect(string code)
        {
            var data = new List<KeyValuePair<string, string>>()
            {
                new KeyValuePair<string, string>("client_id", AppRegistration.ClientId),
            ...
            //var auth = await Post<Auth>("/oauth/token", data);
            string url = "https://" + this.Instance + "/oauth/token";
            ...
            var response = await client.PostAsync(url, content);
            //            return await response.Content.ReadAsStringAsync();
            var auth = TryDeserialize<Auth>(await response.Content.ReadAsStringAsync());
            this.AccessToken = auth.AccessToken;
            return auth;
        }
        ...
        {
            return $"https://{this.Instance}/oauth/authorize?response_type=code&client_id={this.AppRegistration.ClientId}&redirect_uri={redirectUri ?? "urn:ietf:wg:oauth:2.0:oob"}&scope=read write follow";
        }
        ...
        public async Task<IEnumerable<Account>> GetAccountFollowers(int accountId, int limit = 40)
        {
            List<Account> friends = new List<Account>();
            var a = await GetList<IEnumerable<Account>>($"/api/v1/accounts/{accountId}/followers?limit={limit}");

            if (a.Item1 == null) return friends;
            friends.AddRange(a.Item1);
            if (a.Item1.Count() == limit)
            {
                friends.AddRange(await GetAccountFollowers_(accountId, a.Item2, limit));
            }
            return friends;
        }

        private async Task<IEnumerable<Account>> GetAccountFollowers_(int accountId, int max_id, int limit = 40)
        {
            List<Account> friends = new List<Account>();
            var a = await GetList<IEnumerable<Account>>($"/api/v1/accounts/{accountId}/followers?max_id={max_id}&limit={limit}");
            ...
            if (a.Item1.Count() == limit)
            {
                friends.AddRange(await GetAccountFollowing_(accountId, a.Item2, limit));
            }
            return friends;
        }
        ...
        /// </param>
        /// <returns>Returns an array of Accounts</returns>
        public async Task<IEnumerable<Account>> GetAccountFollowing(int accountId, int limit = 40)
        {
            List<Account> friends = new List<Account>();
            var a = await GetList<IEnumerable<Account>>($"/api/v1/accounts/{accountId}/following?limit={limit}");
        ...
        private async Task<IEnumerable<Account>> GetAccountFollowing_(int accountId, int max_id, int limit = 40)
        {
            List<Account> friends = new List<Account>();
            var a = await GetList<IEnumerable<Account>>($"/api/v1/accounts/{accountId}/following?max_id={max_id}&limit={limit}");
        ...
        private async Task<(T, int)> GetList<T>(string route)
        {
            var responce = await GetList(route);

            if (!responce.header.Any(e => e.Key == "Link")) return (TryDeserialize<T>(responce.objects), 0);

            var linkValues = responce.header.FirstOrDefault(e => e.Key == "Link").Value.ToArray()[0];

            var s = linkValues.Split(',');
            for (int i = 0; i < s.Length; i++)
            {
                s[i] = s[i].Trim();
            }
            if (s.Length < 2) return (TryDeserialize<T>(responce.objects), 0);
            var jamauri = s[0].Split(';')[0];// Substring(1,)
            var uri_string = jamauri.Substring(1, jamauri.Length - 2);

            Uri uri = new Uri(uri_string);
            var queris = uri.Query.Split('&');
            Regex regex = new Regex("max_id=*");
            var max_id_queri = queris.FirstOrDefault(regex.IsMatch);
            var max_id = int.Parse(max_id_queri.Split('=').Last());

            return (TryDeserialize<T>(responce.objects), max_id);
        }


        private async Task<(string objects, HttpResponseHeaders header)> GetList(string route)
        {
            string url = "https://" + this.Instance + route;
